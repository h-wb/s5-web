        ...
        <div>
            <div class="global">
                ...
                <br>
                    ...
                    <h3><u>Mes recettes favorites</u></h3></br>

                    <?php
                    echo '<table>';
                    $reponse = $objPDO->query("SELECT idfav FROM favoris WHERE login='".$_SESSION['username']."'");

                    while ($donnees = $reponse->fetch())
                    {
                        $res = $objPDO->prepare("SELECT id, titre From recettes where id=".$donnees['idfav']);
                        $res->execute();
                        ...
                        foreach($result as $key => $result2){
                            if($key === 'id'){
                                ...
                                $titre = $result2;
                                echo '<tr>
                                            <td><a href="page.php?id='.$id.'">' .$titre . '</a></td>
                                        <td>
